...
# Create the time range of the slice
                time_slice_begin = time_duration_cur = self.time_duration_cur
                time_slice_end = min(time_duration_begin + self.time_offset, self.time_duration_end)
                self.time_duration_cur = time_duration_cur + self.time_range
                if self.time_duration_cur > self.time_duration_end:
                    self.time_to_stop = True
                # if not self.time_to_stop:
                    
                return (time_slice_begin, time_slice_end)
