...
class WebScrapyThreadPool(object):
    ...
    def __init__(self, max_num_threads):
        ...
        for _ in range(max_num_threads): 
            worker = web_scrapy_thread.WebScrapyThread(self.tasks)
    ...
    def __add_task(self, web_scrapy_obj, func_name, *args, **kargs):
        ...
        if not isinstance(web_scrapy_obj, web_scrapy_base.WebScrapyBase):
            raise ValueError("web_scrapy_obj should be derived from web_scrapy_base.WebScrapyBase, not %s" % type(web_scrapy_obj))
        ...
        # worker = WebScrapyThread(web_scrapy_obj, self.worker_thread_count)
        # self.worker_thread_list.append(worker)
